...
- 目标版本：`1.19.2-0.0.2.0`
- 状态：代码、静态构建、临时服务端夹具和真实客户端联调均已完成；独立进程旧属性 NBT 回放保留为残余边界
...
- 真实客户端 `/reload`、技能面板加点/洗点和数值变化已由第 5.5 节完成收口。
- 流血 DPS 在真实客户端 `/reload` 前后由 `1.0` 变为 `5.0`，持续时间保持语义已由第 5.5 节完成收口。
- 未做独立进程重启后的旧属性 NBT 回放；夹具完成了 NBT 往返与再次入世校准。
- 旧档中被删除且曾使用非 1 成本的第三方技能无法还原精确历史支付额，只能按 `cost=1` 兜底。
- 本轮不改变网络格式，不需要因 WP-05 重新做双端协议拒绝矩阵。

### 5.5 最终真实客户端联调（2026-09-25 追加）

总工作流第 14 节的真实双端夹具已补齐本 WP 原 5.4 节的前两项残余：

- 技能面板通过真实 `OpenFurkinScreenPacket` 打开；
- 通过真实 `UnlockSkillPacket` 解锁 `sharp_fang`、`bleeding_bite`；
- 真实执行 `/reload`，日志显示流血 DPS 由 `1.0` 变为 `5.0`，持续时间不被改写；
- 洗点后服务端状态为 `points=20 skills={}`，属性 modifier 计数为 0，未出现重复效果。

仍未覆盖独立进程重启后的旧属性 NBT 回放；该项继续作为 WP-05 的残余边界。

---

## 6. 关闭结论

WP-05 的代码、静态构建、服务端夹具、真实客户端联调与文档均已齐备。M-02 的点数账目和加载期 schema 边界已收口，M-04 的已加载实体重建、入世校准和流血实时语义已明确；仅独立进程旧属性 NBT 回放保留为残余边界。
