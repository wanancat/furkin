 ...
 * 调用方只有在收到 {@link Result#ok()} 后才能删除普通档案。</p>
 *
 * <p>清理按冻结顺序执行：AI → 行囊 → 装备/掉率 → 技能效果 → 运行时数据 → 原版归属。
...
public final class FurkinUnbindCleanup {
    ...
    public enum Trigger {
        /** 常规解绑：档案条目仍在。 */
        NORMAL,
        /** 强制解绑后的延迟清理：档案已删除，墓碑仍在。WP-09 使用。 */
        REVOCATION
    }
    ...
    public static final class Result {
        ...
        private Result(boolean success, @Nullable Stage failedStage, @Nullable Throwable cause) {
        ...
        public @Nullable Stage failedStage() {
            return failedStage;
        }
