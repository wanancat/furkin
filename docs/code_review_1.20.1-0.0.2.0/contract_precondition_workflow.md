> [!WARNING]
> 状态：已废弃（2026-09-25）。
> 原因：本文的 §4.2、§4.3、§6、§8 仍混入了 WP-01 之前的确认链路、跨维度活跃上限和版本状态，已不能作为当前实现依据。
> 当前有效文档：`docs/contract_precondition_workflow_v2/README.md`。
> 本文件仅保留历史设计过程与旧版审查证据，不应再据此实施或评估当前源码。
# 契约绒亲前置条件工作流（Enemy / NeutralMob / 两者都没有）

> 状态：设计工作流，血量门槛尚未实现；确认包的服务端权威化前置条件已由 WP-01 于 2026-09-25 完成。
...
> 实施状态（2026-09-25）：WP-01 已落地服务端 pending 会话、目标 UUID/实体 ID/维度绑定、600 tick TTL、单次消费，以及注册表、存活、归属、活跃上限、主手槽位/堆叠、距离和名字复检；真实客户端正常/取消流程已实测。本节下文保留设计期的现状判断，血量门槛仍需在后续实现时按这些权威边界重新校验。
