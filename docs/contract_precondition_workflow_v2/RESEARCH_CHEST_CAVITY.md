...
- 猫、狼 8 点血，30% 只有 2.4 点，玩家需要把宠物打到约 2 点血。
- 第三方大型生物 200 点血，30% 是 60 点，要求打掉 70% 血量。
- 绝对值门槛可以给低血量上限生物一个固定、可理解的底线。

建议只借鉴结构，不照抄数值。默认绝对值应显著低于 10，并按 Furkin 自己目标物种的血量分布选取。
...
- 不要把绝对值设为 10。Furkin 的猫和狼只有 8 点血，会直接让血量门槛失效。
...
如果不采纳双阈值，则保持现有 `30 / 30 / 100` 纯百分比方案，并接受以下体验：满血猫和狼需要被降到约 2 点血才能进入契约命名流程。
