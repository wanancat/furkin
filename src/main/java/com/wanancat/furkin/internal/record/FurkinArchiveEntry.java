...
public final class FurkinArchiveEntry {
    ...
    public void setEntityLocation(Entity entity) {
        ...
        this.entityDimension = entity.level().dimension();
    }

    /** 实体离场或被移除后清空位置，避免后续误用失效定位。 */
    ...
    public static FurkinArchiveEntry deserializeNBT(CompoundTag tag) {
        ...
        if (tag.contains("entity_dimension", CompoundTag.TAG_STRING)) {
            ...
            if (dimensionId == null) {
                ...
                entry.entityDimension = ResourceKey.create(Registries.DIMENSION, dimensionId);
            }
        }
