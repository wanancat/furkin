...
- 状态：计划已建立，**处置口径已于 2026-09-25 冻结**（见第 12 节）；WP-01、WP-02A/B、WP-03、WP-09 已完成代码与专项运行验证；协议升号由紧随其后的 WP-04 收口，其余工作项按 WP 顺序实施中
...
- WP-05：退款修正依赖「技能 ID → cost」的可查性；若技能已从树中移除，需要 WP-05 与 WP-08 共同约定回退口径。
...
**目标**：点数账目准确；效果与技能树永远一致；热重载不留残影、不漂移。

**设计要点**：

1. **退款**：两条路径都改为 `refund += lv * cost(skillId)`；对树中已不存在的技能定义回退口径（例如按 `cost=1` 并在日志告警，或保留历史 cost 快照）。
2. **schema 校验**（`SkillLoader`）：拒绝 `cost < 1`、`maxLevel < 1`、负/零 `tier`、非正 `requiresLevel`、引用不存在的技能；跳过并记日志。
3. **热重载一致**：重载时保留旧树引用，遍历在场宠物「按旧树摘效果 → 按新树重建」；对树中已删除的技能提供按自有 UUID/注册表兜底清除。
4. **流血流快照**：把每秒伤害在**施加时**确定（编码进效果实例数据或 amplifier 派生），使 `/reload` 不影响已存在的流血实例。
5. 更新 `SKILL_TREE.md` 与两份 changelog（若 schema 语义变化）。

**验证**：

- 运行：`/reload`（或数据包重载）后，宠物属性与技能树一致；删除某技能后旧加成消失。
- 运行：`cost != 1` 技能加点 → 洗点 → 点数为 `等级 × cost` 的精确值。
- 运行：施加流血 → `/reload` 改配置 → 已施加效果的 DPS 不漂移。
...
- **D9 的协议升号**：WP-09 已实现强制解绑与墓碑，但 `RecordActionPacket` 语义已变化；必须由 WP-04 把协议从 `"1"` 提升到 `"2"`，在完成前不得宣称 D9 完全关闭。
- **M-04 的旧树引用**：重载时如何安全持有旧树并遍历全部在场宠物，需要确认没有并发/跨维度遗漏。
- **L-01 的真实风险程度**：目前只在服务端启动路径观察无异常，运行期隔离仍未证实，不能据此断言「无风险」。
...
| L-03 | WP-08 | 未开始 |
| L-04 | WP-08 | 未开始 |
| D9 | WP-09 | 代码/夹具完成（2026-09-25，`wp-09_force_unbind_tombstone.md`）；协议升号待 WP-04 |
...
⇒ **严格校验必须以 M-04 的「未知技能兜底清理 + 未知技能退款口径」为前提**，否则加校验本身就是在制造属性与点数残留。WP-05 原本就是 M-02 + M-04 同包，覆盖范围吻合。
...
1. 采用**选项 A：严格校验** —— 违反下列任一规则的技能**不予加载**并记 `WARN`（含引用完整性；即悬空 `requires` / `levelGate`、`requiresLevel > 目标 maxLevel` 也硬拒）：
   - `cost >= 1`、`maxLevel >= 1`、`tier >= 1`、`requiresLevel >= 1`；
   - `requires` / `requiresLevel` / `levelGate` 指向的技能必须存在；
   - `requiresLevel <= 目标技能的 maxLevel`。
