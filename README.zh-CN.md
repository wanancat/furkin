...
- **契约** — 一张道具即可契约任意**已注册**物种，无需事先驯服；敌方、中立和其他分类的生命值门槛均可配置。
- **技能树** — 共享主干 + 物种特色分支。
...
`contractOtherHealthPercent` / `contractOtherHealthAbsolute`。百分比分支与绝对分支任一通过即可；
绝对值设为 `0` 表示禁用该分支，百分比设为 `100` 表示该分类不受百分比限制。
