...
- 状态：实现、构建、资源键校验、启动烟测和游戏内中英文切换验收均已完成
- 日期：2026-09-25
- 基线：WP-06 后的工作树
...
- 游戏内中英文切换及召唤失败、列表路径已由第 6 节完成收口；固定键存在性、动态键存在性和代码路径已静态核对。
- `not_owner`、损坏 capability、幽灵档案等异常分支仍需要第二玩家或故障夹具，属于 L-02 之外的功能验证边界。
- 命令行的玩家自定义名字保持字面量，符合“动态输入不翻译”的口径。

## 6. 最终双语命令验收（2026-09-25 追加）

真实客户端在游戏内切换 `zh_cn` 后执行 `furkin summon not-a-uuid` 与 `furkin list`，聊天栏显示中文回执；切换 `en_us` 后执行同一命令，聊天栏显示 `Invalid pet id: not-a-uuid`、`Your companions:` 与 `(none)`。同一客户端进程完成了两次资源重载，未出现资源键缺失或项目包 `ERROR` / `FATAL`。

本结论只覆盖本轮执行到的召唤失败与列表路径；其余异常分支（第二玩家、损坏 capability、幽灵档案）仍按本 WP 第 5 节边界处理。
