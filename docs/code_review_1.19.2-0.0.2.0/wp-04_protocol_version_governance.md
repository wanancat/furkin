...
- 状态：代码、静态映射和专用服务端启动验证完成；真实同版本双端握手验证在客户端联调步骤补证
...
同版本客户端 → 服务端握手需要在 WP-09 客户端确认页完成后统一执行。当前可以确认的 Forge 语义是：
...
协议 `"2"` 与当前 `0-10` 包结构已绑定，M-01 的代码边界已建立。WP-04 的协议文档、静态映射和专用服务端启动验证完成；最终客户端同版本握手与界面联调在 WP-06 后收口。
